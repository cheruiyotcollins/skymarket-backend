...
class CartItemControllerTest {
    ...
    void addOrUpdateCartItem_ShouldReturnResponseEntity() {
        ...
        // Mock service method with all required arguments
        when(cartItemService.addOrUpdateCartItem(Optional.of(cart), Optional.of(product), 2, mockUserPrincipal))
        ...
        verify(cartItemService, times(1)).addOrUpdateCartItem(Optional.of(cart), Optional.of(product), 2, mockUserPrincipal);
    }
    ...
    void getAllCartItems_ShouldReturnResponseEntity() {
        ...
        Long cartId = 1L;
        int page = 0;
        int size = 20; // Match the default value in the controller method
        String sort = "id,asc";
        ResponseDto responseDto = new ResponseDto();
        ResponseEntity<ResponseDto> expectedResponse = new ResponseEntity<>(responseDto, HttpStatus.OK);

        // Mock service method with correct parameters
        when(cartItemService.getAllCartItems(cartId, page, size, sort)).thenReturn(expectedResponse);

        // Act
        ResponseEntity<ResponseDto> actualResponse = cartItemController.getAllCartItems(cartId, page, size, sort);

        // Assert
        assertEquals(expectedResponse, actualResponse);
        verify(cartItemService, times(1)).getAllCartItems(cartId, page, size, sort);
    }
